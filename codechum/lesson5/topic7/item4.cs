...
public class SemestralSchedule : ScheduleStructure {
  ...
  public SemestralSchedule(DateTime currentDate, DateTime sem1Start,
                           DateTime sem1End, DateTime sem2Start,
                           DateTime sem2End, int currentPeriodNumber)
  ...
  public override int GetDaysUntilNextPeriod() {
    ...
    if (periodIndex ==
        0) // If it's the end of the second semester, adjust for the next year
    {
      nextPeriodStart = new DateTime(
          pairs[1][1].Year + 1, nextPeriodStart.Month, nextPeriodStart.Day);
    }
...
public class SchoolYearSchedule : ScheduleStructure {
  ...
  public SchoolYearSchedule(DateTime currentDate, DateTime start, DateTime end)
      : base(1) // School year schedule does not use period numbers, so default
                // to 1
  {
    this.start = start;
    this.end = end;
