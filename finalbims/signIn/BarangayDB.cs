...
internal class BarangayDB {
  ...
  public void AddNewBarangay(string barangayName, string address,
                             string description, byte[] BarangayLogo) {
    using (SqlConnection conn = new SqlConnection("connectionString")) {
      ...
      using (SqlCommand cmd = new SqlCommand("YourStoredProcName", conn)) {
        ...
        if (BarangayLogo != null) {
          cmd.Parameters.Add("@BarangayLogo", SqlDbType.VarBinary, -1).Value =
              BarangayLogo;
        } else {
          cmd.Parameters.Add("@BarangayLogo", SqlDbType.VarBinary, -1).Value =
              DBNull.Value;
