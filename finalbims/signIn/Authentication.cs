...
public bool SignUp(string username, string emailAddress, string password) {
  ...
  using (var connection = new SqlConnection(connectionString)) {
    ...
    var query = "INSERT INTO SystemUsers (Username, EmailAddress, " +
                "PasswordHash, IsActive, CreatedAt) VALUES (@Username, " +
                "@EmailAddress, @PasswordHash, 1, GETDATE())";
...
private bool UserExists(string username, string emailAddress) {
  using (var connection = new SqlConnection(connectionString)) {
    connection.Open();
    var query = "SELECT COUNT(1) FROM SystemUsers WHERE Username = @Username " +
