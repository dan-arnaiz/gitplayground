...
internal class BarangayDB {
  private string
      connectionString = @"Server=localhost;Database=dan-bims;Integrated " +
                         @"Security=True;"; // Update with your actual
                                            // connection string

  public async
      Task RefreshBarangaysTableAsync(DataGridView barangaysDatatable) {
    string query = @"SELECT BarangayID, BarangayLogo, BarangayName, Address, " +
                   @"Description FROM Barangays";
  ...
  public async Task AddNewBarangay(byte[] BarangayLogo, string barangayName,
                                   string address, string description) {
    string query =
        @"INSERT INTO Barangays (BarangayLogo, BarangayName, Address, Description)
                 VALUES (@BarangayLogo, @BarangayName, @Address, @Description)";

    using (SqlConnection conn = new SqlConnection(connectionString)) {
      await conn.OpenAsync();
      using (SqlCommand cmd = new SqlCommand(query, conn)) {
        cmd.Parameters.Add("@BarangayLogo", SqlDbType.Image).Value =
            BarangayLogo;
  ...
  UpdateBarangayAsync(string BarangayID, byte[] BarangayLogo,
    ...
    using (SqlConnection conn = new SqlConnection(connectionString)) {
      ...
      using (SqlCommand cmd = new SqlCommand(query, conn)) {
        cmd.Parameters.Add("@BarangayLogo", SqlDbType.Image).Value =
            BarangayLogo ?? (object)DBNull.Value; // Handle null images
        cmd.Parameters.AddWithValue("@BarangayName", barangayName);
        cmd.Parameters.AddWithValue("@Address", address);
        cmd.Parameters.AddWithValue("@Description", description);
        cmd.Parameters.AddWithValue("@BarangayID", BarangayID);
        ...
        int result = await cmd.ExecuteNonQueryAsync();
        return result >
               0; // Returns true if the update was successful, false otherwise
      }
    }
  }
}
