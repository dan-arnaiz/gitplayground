...
public partial class ControllerEditBarangayModal : UserControl
{
    ...
    private async void applyChangesButtonBarangay_Click(object sender, EventArgs e)
    {
        InitializeComponent();
        this.barangaysDatatable.CellClick += new DataGridViewCellEventHandler(this.barangaysDatatable_CellContentClick);
        // Assume you have textboxes for ID, name, address, description, and a PictureBox for the logo
        string barangayID = barangayIDDataGridViewTextBoxColumn.Text;
        byte[] barangayLogo = BarangayDB.ImageToByteArray(BarangayLogoImageChange); // Assuming barangayDB is an instance of BarangayDB
        ...
        bool success = await BarangayDB.UpdateBarangayAsync(barangayID, barangayLogo, barangayName, address, description);
