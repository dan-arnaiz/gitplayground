...
public partial class ControllerEditBarangayModal : UserControl {
  ...
  barangaysDatatable_CellContentClick(object sender,
    ...
    if (e.RowIndex >= 0) {
      ...
      // Get the 'barangayNameDataGridViewTextBoxColumn',
      // 'addressDataGridViewTextBoxColumn', and
      // 'descriptionDataGridViewTextBoxColumn' column values of the clicked row
  ...
  private async void applyChangesButtonBarangay_Click(object sender,
                                                      EventArgs e) {
    // Assuming barangayIDDataGridViewTextBoxColumn, BarangayNameEditTextBox,
    // addressTextBox, and descriptionTextBox are the control names
    string barangayID = barangayIDDataGridViewTextBoxColumn.Text;
    byte[] barangayLogo = BarangayDB.ImageToByteArray(
        BarangayLogoImageChange); // Convert image to byte array
    ...
    bool success = await db.UpdateBarangayAsync(
